/*******************************************************************************/
/* _1632LEDPanelBasicPortmanipulation - Simpler Sketch zum Debuggen der
   ...
   Zeitintensive digitalWrite() in der loop() durch Portmanipulation ersetzt.
...
const char* sketchname            =  "_1632LEDPanelBasicPortmanipulation";
const char* revision              =  "R.1.0";
const char* author                =  "Olaf Meier";
const char* date                  =  "2015/12/02";
...
byte brightPercent                =  1;           // Helligkeit 0% - 100%
...
void setup() {
  ...
  /*****************************************************************************/
  /***  Schieberegister / Latch mit RGB Daten, hier mit LOW vorbelege  ***/
  ...
  /***  Im oberen Segment wird die Farbe Cyan, im unteren Gelb angezeigt     ***/
...
void loop() {
  ...
  for (int i = 0; i <= 7; i++)  {                  // Zeile 1 bis 8, 9 bis 16
    ...
    //delay(100);                                  // Optional Delay for debugging!
    brightness(brightPercent);                     // Brightness, 0% bis 100%
  }
}                                                  // Ende Loop (Endlos)
/*******************************************************************************/
/*******************************************************************************/
...
/*******************************************************************************/
/*******************************************************************************/
/***  Funktion zur Helligkeitssteuerung über den Pin /OE von 0 bis 100%  ***/
void brightness(uint16_t _onOffRatio) {
  uint16_t maxOffTime;
  byte usFactor = 10;                              // Faktor 1 - 165 für delay us
  /***  Bei 0% Dutycycle das Panel generell ausschalten  ***/ 
  if (_onOffRatio == 0) {                          // Bei 0% Panel ausschalten
    digitalWrite(OE, HIGH);                        // Panel ausschalten.
  }
  /***  Bei 100% Dutycycle bleibt das Panel generell eingeschaltet  ***/ 
  else if(_onOffRatio == 100) {                    // Bei 100% Panel ein
    digitalWrite(OE, LOW);                         // Panel einschalten
  }
  else {
  /***  Steuere die ON/Off-Zeit des Panels über den /OE-Eingang via SW-PWM  ***/
    _onOffRatio *= usFactor;                       // Berechne Einschaltzeit
    maxOffTime = (100 * usFactor);                 // Berechne Ausschaltzeit
    /***  Panel einschalten ***/
    //digitalWrite(OE, LOW);                       // Panel einschalten
    PORTB &= B11111101;                            // Setze Pin 9 (/OE) auf LOW
    delayMicroseconds(_onOffRatio);
    /***  Panel auschalten  ***/
    //digitalWrite(OE, HIGH)                       // Panel ausschalten.
    PORTB |= B00000010;                            // Setze Pin 9 (/OE) auf HIGH
    delayMicroseconds(maxOffTime - _onOffRatio);
  }
}                                                  // Ende der Funktion brightness
/*******************************************************************************/
/*******************************************************************************/
